  ...
  <div class="layout">
    ...
    <div class="mobile-bar">
      ...
      <div class="mobile-actions">
        ...
        <!-- View Public Page -->
        <a v-if="userStore.currentUser" :href="`/p/${userStore.currentUser.username}`" target="_blank" class="mobile-theme-btn" aria-label="View public page">
          <ExternalLink :size="16" />
        </a>

        <!-- Mobile Theme Toggle -->
        <button class="mobile-theme-btn" :aria-label="theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'" @click="toggleTheme">
...
<style scoped>
  .layout {
    @apply min-h-dvh bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white flex overflow-x-hidden;
  }
  .sidebar {
    @apply w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-col fixed h-full hidden md:flex;
  }
  .sidebar-header {
  ...
  .sidebar-action-btn {
    @apply flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-500 dark:text-gray-400
           hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white transition-colors w-full
  ...
  .main-content {
    @apply flex-1 md:ml-64 px-4 md:px-8 py-6 md:py-10 mt-16 md:mt-0 bg-gray-50 dark:bg-gray-950 overflow-x-hidden;
  }
</style>
